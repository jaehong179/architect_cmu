...
> **메모리 비용**: 8분 이력은 원신호(raw) 풀해상도 보관이 지배적이다 — 48 kHz mono float
> 기준 `8분×60×48 000×4 B ≈ **92 MB**`(+엔벨로프 LOD·이벤트는 그보다 훨씬 작음). "메모리보다
> 정확도/속도 우선"이라 raw 를 데시메이션 없이 들고, 렌더만 LOD 로 추린다(A안).
