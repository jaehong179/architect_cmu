...
> **왜 탭별 `WaveBuffer` 를 중앙 하나로 합치지 않았나** — 8분 *이력*은 의도적으로 **중앙
> `WaveLodHistory` 하나**로 모았다(탭마다 8분×raw 를 들면 13×92 MB 라 불가 → "한 버퍼를 두고
> 탭이 가져가는" 구조). 반면 **라이브 렌더용 `WaveBuffer` 는 탭별로 남겼다** — 각 탭이 매 프레임
> 자기 형식(엔벨로프/raw · 창 크기 · C 정렬 등)의 최근 구간을 싸게 그려야 하기 때문이다.
> **seek 시엔 중앙이 탭별 `WaveBuffer` 로 그 구간을 `replayInto` 해 채운다**(중앙 = 8분 원천,
> 탭별 = 렌더 작업버퍼). 즉 "하나의 버퍼 + 탭이 가져가는 pub-sub"은 **이력/seek 경로**에 적용되고,
> 라이브 경로는 기존 탭별 버퍼를 그대로 쓴다.
