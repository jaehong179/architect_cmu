...
Letting every display hold its own multi-minute history would multiply memory by the number of displays and worsen render scaling — the growth captured in RISK-03. A single central store bounds memory regardless of display count and keeps long-run resource use predictable, which QAS-03 requires. Routing live data through the central store and having each display re-pull and re-format it every frame would add work on the hot path, threatening the end-to-end latency target (QAS-02) and the timing-precision concern of extra buffering/resampling (RISK-18). Keeping the live path direct avoids this; the history accumulates alongside as a cheap append, preserving streaming throughput (QAS-01). Introducing the history as a non-visual subscriber confines the change to one integration point with no edits to core processing or existing displays — the same open/closed property as QAS-08 and a continuation of ADR-003. Retaining the signal at full resolution while applying level-of-detail only at draw time preserves zoom/measurement fidelity yet bounds the number of plotted points, so render cost stays bounded as the window grows (RISK-03).
...
- Memory is bounded by one central buffer instead of (display count × history), keeping long-run resource use predictable — satisfies QAS-03 and mitigates RISK-03.
...
- The live path stays direct and unchanged, protecting end-to-end latency and throughput (QAS-02, QAS-01) and avoiding added timing risk (RISK-18).
- All displays see the same history, eliminating inconsistencies when different displays show the same seek position.
- History is added at a single integration point with no changes to core processing or existing displays (QAS-08; builds on ADR-003).
...
- Full-resolution retention keeps a large resident memory footprint; its long-run effect on resource/thermal stability must be verified against QAS-03 (sustained-operation resource/thermal spikes).
