...
Without a registration point, each new tab needs edits scattered across the layout, sibling tabs, and sometimes DSP — the "module change explosion" of [RISK-06](../README.md#risk-06). Routing every tab through registerTab() inverts the dependencies: the manager and the tabs depend on the interface, not on each other. This confines change to the new tab plus one line, exactly what [QAS-08](../README.md#qas-08-new-tab-extensibility) requires.
...
- Satisfies [QAS-08](../README.md#qas-08-new-tab-extensibility): a new tab needs one registerTab() call and no DSP/existing-tab changes, so regressions stay at zero.
